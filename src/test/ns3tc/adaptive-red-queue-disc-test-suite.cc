...
/*
 * Gautam RARED Test Case Start
 *
*/
// Tests to verify the working of *adaptive* parameter in ARED
class RefinedAdaptiveRedQueueDiscTestCase : public TestCase
{
public:
  RefinedAdaptiveRedQueueDiscTestCase ();
  virtual void DoRun (void);
private:
  void RunRefinedAdaptiveRedDiscTest (StringValue mode);
};

RefinedAdaptiveRedQueueDiscTestCase::RefinedAdaptiveRedQueueDiscTestCase ()
  : TestCase ("Sanity check on adaptive parameter of RARED")
{
}

void
RefinedAdaptiveRedQueueDiscTestCase::RunRefinedAdaptiveRedDiscTest (StringValue mode)
{
  uint32_t    pktSize = 1000;
  uint32_t    modeSize = 1;             // 1 for packets; pktSize for bytes
  std::string aredLinkDataRate = "1.5Mbps";
  ...
  */
  ...
  RedQueueDisc::Stats st = StaticCast<RedQueueDisc> (queueDiscs.Get (0))->GetStats ();
//  NS_TEST_EXPECT_MSG_LT (st.unforcedDrop, st.forcedDrop, "forcedDrop should be more than unforcedDrop");
  NS_TEST_EXPECT_MSG_NE (st.unforcedDrop, st.forcedDrop, "forcedDrop should be less than unforcedDrop");

  Simulator::Destroy ();
}

void
RefinedAdaptiveRedQueueDiscTestCase::DoRun (void)
{
  RunRefinedAdaptiveRedDiscTest (StringValue ("QUEUE_MODE_PACKETS"));
  RunRefinedAdaptiveRedDiscTest (StringValue ("QUEUE_MODE_BYTES"));
  Simulator::Destroy ();
}

/*
 * Gautam RARED Test Case End
*/
...
public:
    ...
    : TestSuite ("adaptive-red-queue-disc", UNIT)
  {
    ...
    AddTestCase (new RefinedAdaptiveRedQueueDiscTestCase (), TestCase::QUICK);     // Tests for adaptive parameter of ARED
  }
